...
var environment = config['devops_api-cicd_api-srinivas_api'].env;
...
console.log('&$devops_api-cicd_api-srinivas_api api: [' +  domains[environment] + ', ' + config['devops_api-cicd_api-srinivas_api'].basepath + ']');

BeforeAll("BeforeFeatures", function(next) {
    if (apps[0].name) {
        console.log ("Getting Creds from DevAppsKeys.json");
        getCreds(config['devops_api-cicd_api-srinivas_api'].app, config['devops_api-cicd_api-srinivas_api'].product);
    } else
    {
...
Before(function(scenario, callback) {
    ...
    this.apickli = new apickli.Apickli(tokens[0],
        tokens[1]+ config['devops_api-cicd_api-srinivas_api'].basepath,
        './test/integration/features/fixtures/');
    callback();
});
